...
void smallslide() {
  ...
  while (run_mode()) {
    ...
    if (rtc1[0] != old_secs) {
      ...
      digits_new[1] = ((rtc1[0] / 10) % 10);  //1 - secs tens
      //rtc1[1] = mins
      digits_new[2] = (rtc1[1] % 10);         //2 - mins ones
      digits_new[3] = ((rtc1[1] / 10) % 10);  //3 - mins tens
      //rtc1[2] = hours
...
/*
  ...
  while (run_mode()) {
    ...
    if (circle) {
      //if(rtc1[1] % 2 == 1 && (rtc1[0] >= 14 && rtc1[0] <=30 )){
      // we skip word mode for now
      //
      //
      //
      if (true) {
    ...
    if ((rtc1[0] >= 35 && rtc1[0] <= 45) && date_state) {
      display_date();
      return;
    }
    else {
...
void shift() {
  ...
  while (run_mode()) {
    ...
    if (circle) {
      if (rtc1[1] % 2 == 1 && (rtc1[0] >= 4 && rtc1[0] <= 15 )) {
        wipeMiddle();
        display_temp();
        wipeTop();
...
void get_shiftTime(bool secflag) {
  ...
  if (secflag) {
    if (rtc1[0] >= 4) {
      rtc1[0] = rtc1[0] - 4; // -4 seconds, because the shifting of previous chars was approx. 4 seconds in the past
    }
  }
  ...
  char buffer_hours[3];
  itoa( rtc1[2], buffer_hours, 10);
  char buffer_mins[3];
  itoa( rtc1[1], buffer_mins, 10);
  char buffer_secs[3];
  itoa( rtc1[0], buffer_secs, 10);
...
void display_date() {
  ...
  byte dow = rtc1[3]; // day of week 0 = Sunday
  byte date = rtc1[4];
  byte month = rtc1[5] - 1;
  byte year = rtc1[6] - 2000;
...
void toggleDateState() {
  if (show_date == true ) {
    ...
    date_state = false;
    if (debug) {
      Serial.println(F("Show date = Off"));
    }
    //cls();
    wipeTop();
    //display state of date
...
void set_time() {
  ...
  byte set_min   = rtc1[1];
  byte set_hr    = rtc1[2];
  byte set_date  = rtc1[4];
  byte set_mnth  = rtc1[5];
  int  set_yr    = rtc1[6];
  ...
  rtc.adjust(DateTime(set_yr, set_mnth, set_date, set_hr, set_min));
  ...
  /*
    ...
    #if XDS3231==1
    set_yr = set_yr - 2000;
    Clock.setYear(set_yr);
    Clock.setMonth(set_mnth);
    Clock.setDate(set_date);
    //Clock.setDoW(DoW);
    Clock.setHour(set_hr);
    Clock.setMinute(set_min);
    //Clock.setSecond(Second);
    #endif
  */
  if (debug) {
    //print the time to the serial port - for debuging
    Serial.print(F("Set day month year > hr min:"));
    Serial.print(set_date);
    Serial.print(F(" "));
    Serial.print(set_mnth);
    Serial.print(F(" "));
    Serial.print(set_yr);
...
void get_time()
{
  ...
  rtc1[5] = now.month();
  rtc1[4] = now.day();
  rtc1[3] = now.dayOfWeek(); //returns 0-6 where 0 = Sunday
  //rtc1[2] = now.hour();  depends on boolean summertime_EU
  rtc1[2] = now.hour();
  rtc1[1] = now.minute();
  rtc1[0] = now.second();
  //#endif
  ...
  /*
    //save time to array
    rtc1[6] = 2000 + Clock.getYear();
    bool Century = false;
    rtc1[5] = Clock.getMonth(Century);
    rtc1[4] = Clock.getDate();
    rtc1[3] = Clock.getDoW();; //returns 0-6 where 0 = Sunday

    bool h12;
    bool PM;
    rtc1[2] = Clock.getHour(h12, PM);
    rtc1[1] = Clock.getMinute();
    rtc1[0] = Clock.getSecond();
  */
  //#endif
  /*
    ...
    if (summertime_EU(now.year(), now.month(), now.day(), now.hour(), 1)) {
      ...
      rtc1[2] = now.hour() + 1;  // wintertime: add 1 hour
      if (rtc1[2] > 23) {      // if hour > 23, we have next day between 00:00:00 and 01:00:00
        rtc1[2] = 0;           // hour = 0
        rtc1[3] = rtc1[3] + 1;  // dayOfWeek +1
        rtc1[4] = rtc1[4] + 1;  // day +1
      }
    }
    else {
      if (debug) {
      ...
      rtc1[2] = now.hour();    // summertime = "normal" hour
    }

  */

  // Calculate day of year and week of year
  DayWeekNumber(rtc1[6], rtc1[5], rtc1[4], rtc1[3]);
